...
/**
 * Routing class from com_spbrand
 *
 * @since  3.3
 */
class SpbrandRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spbrand component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'brand' || $view === 'listbrand' || $view === 'itembrand'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'brand' || $view === 'listbrand' || $view === 'itembrand'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'listbrand':
				...
				{
					$id = $this->getVar('brand', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spbrand')
	{
...
function SpbrandBuildRoute(&$query)
{
	$router = new SpbrandRouter;
	
	return $router->build($query);
}

function SpbrandParseRoute($segments)
{
	$router = new SpbrandRouter;
