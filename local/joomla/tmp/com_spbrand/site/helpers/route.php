...
/**
 * Spbrand Route Helper
 **/
abstract class SpbrandHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listbrand
	 */
	public static function getListbrandRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listbrand'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spbrand&view=listbrand&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spbrand&view=listbrand';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spbrand category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spbrand&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spbrand');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpbrandHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
