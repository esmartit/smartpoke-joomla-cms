...
/**
 * Spvalue Route Helper
 **/
abstract class SpvalueHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listvalue
	 */
	public static function getListvalueRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listvalue'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spvalue&view=listvalue&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spvalue&view=listvalue';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spvalue category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spvalue&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spvalue');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpvalueHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
