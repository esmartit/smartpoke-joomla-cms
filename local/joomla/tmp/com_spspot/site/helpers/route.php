 ...
 * Spspot Route Helper
 **/
abstract class SpspotHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listspot
	 */
	public static function getListspotRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listspot'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spspot&view=listspot&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spspot&view=listspot';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spspot category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spspot&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spspot');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpspotHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
