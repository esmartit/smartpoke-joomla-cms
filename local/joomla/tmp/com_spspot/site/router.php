...
/**
 * Routing class from com_spspot
 *
 * @since  3.3
 */
class SpspotRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spspot component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'spot' || $view === 'listspot' || $view === 'itemspot'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'spot' || $view === 'listspot' || $view === 'itemspot'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'listspot':
				...
				{
					$id = $this->getVar('spot', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spspot')
	{
...
function SpspotBuildRoute(&$query)
{
	$router = new SpspotRouter;
	
	return $router->build($query);
}

function SpspotParseRoute($segments)
{
	$router = new SpspotRouter;
