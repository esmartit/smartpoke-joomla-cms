...
/**
 * Spcampaign Route Helper
 **/
abstract class SpcampaignHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listcampaign
	 */
	public static function getListcampaignRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listcampaign'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spcampaign&view=listcampaign&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spcampaign&view=listcampaign';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spcampaign category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spcampaign&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spcampaign');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpcampaignHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
		...
		if ($active
			&& $active->component == 'com_spcampaign'
