...
/**
 * Routing class from com_spzone
 *
 * @since  3.3
 */
class SpzoneRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spzone component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'zone' || $view === 'listzone' || $view === 'itemzone'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'zone' || $view === 'listzone' || $view === 'itemzone'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'listzone':
				...
				{
					$id = $this->getVar('zone', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spzone')
	{
...
function SpzoneBuildRoute(&$query)
{
	$router = new SpzoneRouter;
	
	return $router->build($query);
}

function SpzoneParseRoute($segments)
{
	$router = new SpzoneRouter;
