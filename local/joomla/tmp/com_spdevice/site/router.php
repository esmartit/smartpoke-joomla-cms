...
/**
 * Routing class from com_spdevice
 *
 * @since  3.3
 */
class SpdeviceRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spdevice component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'device' || $view === 'listdevice' || $view === 'itemdevice'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'device' || $view === 'listdevice' || $view === 'itemdevice'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'listdevice':
				...
				{
					$id = $this->getVar('device', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spdevice')
	{
...
function SpdeviceBuildRoute(&$query)
{
	$router = new SpdeviceRouter;
	
	return $router->build($query);
}

function SpdeviceParseRoute($segments)
{
	$router = new SpdeviceRouter;
