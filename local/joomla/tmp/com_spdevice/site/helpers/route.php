...
/**
 * Spdevice Route Helper
 **/
abstract class SpdeviceHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listdevice
	 */
	public static function getListdeviceRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listdevice'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spdevice&view=listdevice&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spdevice&view=listdevice';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spdevice category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spdevice&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spdevice');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpdeviceHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
