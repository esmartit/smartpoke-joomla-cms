...
/**
 * Spzipcode component helper.
 */
abstract class SpzipcodeHelper
{
	/**
	 * Composer Switch
	 * 
	...
	{
		$manifestUrl = JPATH_ADMINISTRATOR."/components/com_spzipcode/spzipcode.xml";
	...
	{
		// get params
		$params	= JComponentHelper::getParams('com_spzipcode');
	...
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_SPZIPCODE_SUBMENU_DASHBOARD'), 'index.php?option=com_spzipcode&view=spzipcode', $submenu === 'spzipcode');
		if ($user->authorise('zipcode.access', 'com_spzipcode') && $user->authorise('zipcode.submenu', 'com_spzipcode'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_SPZIPCODE_SUBMENU_ZIPCODES'), 'index.php?option=com_spzipcode&view=zipcodes', $submenu === 'zipcodes');
		}
	}
	...
	*/
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'spzipcode')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'spzipcode', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__spzipcode_'.$type.' AS a');
	 ...
	 * 
	 */
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'spzipcode', $user = 'null')
	{
	 ...
	 */
	public static function getModel($name, $path = JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Spzipcode', $config = array())
	{
		// fix the name
	...
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_spzipcode');
		
		$parentId = $parent->id;
		$name     = 'com_spzipcode.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_spzipcode', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__spzipcode_'.$table, $object, 'id');
			}
		}
		return false;
	}
	...
	public static function transliterate($string)
	{
		...
		{
			// get global value
			self::$langTag = JComponentHelper::getParams('com_spzipcode')->get('language', 'en-GB');
		}
