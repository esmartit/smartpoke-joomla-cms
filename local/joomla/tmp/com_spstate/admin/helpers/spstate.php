...
/**
 * Spstate component helper.
 */
abstract class SpstateHelper
{
	/**
	 * Composer Switch
	 * 
	...
	{
		$manifestUrl = JPATH_ADMINISTRATOR."/components/com_spstate/spstate.xml";
	...
	{
		// get params
		$params	= JComponentHelper::getParams('com_spstate');
	...
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_SPSTATE_SUBMENU_DASHBOARD'), 'index.php?option=com_spstate&view=spstate', $submenu === 'spstate');
		if ($user->authorise('state.access', 'com_spstate') && $user->authorise('state.submenu', 'com_spstate'))
		{
			JHtmlSidebar::addEntry(JText::_('COM_SPSTATE_SUBMENU_STATES'), 'index.php?option=com_spstate&view=states', $submenu === 'states');
		}
	}
	...
	*/
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'spstate')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'spstate', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__spstate_'.$type.' AS a');
	 ...
	 * 
	 */
	public static function getActions($view, &$record = null, $views = null, $target = null, $component = 'spstate', $user = 'null')
	{
	 ...
	 */
	public static function getModel($name, $path = JPATH_COMPONENT_ADMINISTRATOR, $Component = 'Spstate', $config = array())
	{
		// fix the name
	...
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_spstate');
		
		$parentId = $parent->id;
		$name     = 'com_spstate.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_spstate', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__spstate_'.$table, $object, 'id');
			}
		}
		return false;
	}
	...
	public static function transliterate($string)
	{
		...
		{
			// get global value
			self::$langTag = JComponentHelper::getParams('com_spstate')->get('language', 'en-GB');
		}
