...
/**
 * Routing class from com_sphotspot
 *
 * @since  3.3
 */
class SphotspotRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_sphotspot component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'listhotspot'))
		{
	...
	{		
		...
		switch($segments[0])
		{
			case 'listhotspot':
				...
				{
					$id = $this->getVar('hotspot', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'sphotspot')
	{
...
function SphotspotBuildRoute(&$query)
{
	$router = new SphotspotRouter;
	
	return $router->build($query);
}

function SphotspotParseRoute($segments)
{
	$router = new SphotspotRouter;
