...
/**
 * Routing class from com_spcustomer
 *
 * @since  3.3
 */
class SpcustomerRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spcustomer component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'customer' || $view === 'listcustomer' || $view === 'itemcustomer'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'customer' || $view === 'listcustomer' || $view === 'itemcustomer'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			...
			case 'listcustomer':
				...
				{
					$id = $this->getVar('customer', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spcustomer')
	{
...
function SpcustomerBuildRoute(&$query)
{
	$router = new SpcustomerRouter;
	
	return $router->build($query);
}

function SpcustomerParseRoute($segments)
{
	$router = new SpcustomerRouter;
