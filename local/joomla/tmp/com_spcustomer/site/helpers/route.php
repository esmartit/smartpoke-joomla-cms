...
/**
 * Spcustomer Route Helper
 **/
abstract class SpcustomerHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listcustomer
	 */
	public static function getListcustomerRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listcustomer'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spcustomer&view=listcustomer&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spcustomer&view=listcustomer';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spcustomer category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spcustomer&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spcustomer');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpcustomerHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
		...
		if ($active
			&& $active->component == 'com_spcustomer'
