 ...
 */
class SpcountryTableCountry extends JTable
{
	/**
	...
	{
		parent::__construct('#__spcountry_country', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_spcountry.country'));
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('country', 'SpcountryTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SPCOUNTRY_COUNTRY_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_spcountry.country.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_spcountry.country.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_spcountry');
