...
/**
 * Spbusiness Route Helper
 **/
abstract class SpbusinessHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listbusiness
	 */
	public static function getListbusinessRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listbusiness'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spbusiness&view=listbusiness&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spbusiness&view=listbusiness';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spbusiness category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spbusiness&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spbusiness');
		...
		if ($needles)
		{
			foreach ($needles as $view => $ids)
			{
				if (isset(self::$lookup[$language][$view]))
				{
					if (SpbusinessHelper::checkArray($ids))
					{
						foreach ($ids as $id)
						{
		...
		if ($active
			&& $active->component == 'com_spbusiness'
