...
/**
 * Routing class from com_spmessage
 *
 * @since  3.3
 */
class SpmessageRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spmessage component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'listmessage'))
		{
	...
	{		
		...
		switch($segments[0])
		{
			case 'listmessage':
				...
				{
					$id = $this->getVar('message', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spmessage')
	{
...
function SpmessageBuildRoute(&$query)
{
	$router = new SpmessageRouter;
	
	return $router->build($query);
}

function SpmessageParseRoute($segments)
{
	$router = new SpmessageRouter;
