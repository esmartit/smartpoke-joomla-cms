 ...
 * Messages Model
 */
class SpmessageModelMessages extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$senddate = $this->getUserStateFromRequest($this->context . '.filter.senddate', 'filter_senddate');
		$this->setState('filter.senddate', $senddate);
	...
	public function selectionTranslation($value,$name)
	{
		...
		{
			$statusArray = array(
				1 => 'COM_SPMESSAGE_MESSAGE_OK',
				0 => 'COM_SPMESSAGE_MESSAGE_NOK'
			);
			// Now check if value is found in this array
			if (isset($statusArray[$value]) && SpmessageHelper::checkString($statusArray[$value]))
			{
	...
	protected function getListQuery()
	{
		...
		if (!$user->authorise('core.options', 'com_spmessage'))
		{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.device_sms LIKE '.$search.' OR a.username LIKE '.$search.')');
			}
		}
		...
		{
			$query->where('a.device_sms = ' . $db->quote($db->escape($device_sms)));
		}
		// Filter by Username.
		if ($username = $this->getState('filter.username'))
		{
			$query->where('a.username = ' . $db->quote($db->escape($username)));
		}
		// Filter by Status.
		if ($status = $this->getState('filter.status'))
		{
			$query->where('a.status = ' . $db->quote($db->escape($status)));
		}
	...
	{
		// setup the query
		if (SpmessageHelper::checkArray($pks))
		{
			// Set a value to know this is export method. (USE IN CUSTOM CODE TO ALTER OUTCOME)
			$_export = true;
			// Get the user object if not set.
			if (!isset($user) || !SpmessageHelper::checkObject($user))
			{
				$user = JFactory::getUser();
			}
			// Create a new query object.
			...
			// From the spmessage_message table
			$query->from($db->quoteName('#__spmessage_message', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__spmessage_message");
		if (SpmessageHelper::checkArray($columns))
		{
	...
	protected function checkInNow()
	{
		...
		$time = JComponentHelper::getParams('com_spmessage')->get('check_in');
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__spmessage_message'))->set($fields)->where($conditions); 
