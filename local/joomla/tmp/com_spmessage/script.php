...
/**
 * Script File of Spmessage Component
 */
class com_spmessageInstallerScript
{
	/**
	 * Constructor
	 *
	...
	public function uninstall(JAdapterInstance $parent)
	{
		...
		$query->where( $db->quoteName('type_alias') . ' = '. $db->quote('com_spmessage.message') );
		$db->setQuery($query);
		...
		if ($message_found)
		{
			...
			$message_condition = array( $db->quoteName('type_alias') . ' = '. $db->quote('com_spmessage.message') );
			// Create a new query object.
			$query = $db->getQuery(true);
			...
			if ($message_done)
			{
				// If succesfully remove Message add queued success message.
				$app->enqueueMessage(JText::_('The (com_spmessage.message) type alias was removed from the <b>#__content_type</b> table'));
			}
			...
			{
				// If succesfully remove Message add queued success message.
				$app->enqueueMessage(JText::_('The (com_spmessage.message) type alias was removed from the <b>#__contentitem_tag_map</b> table'));
			}

			// Remove Message items from the ucm content table
			$message_condition = array( $db->quoteName('core_type_alias') . ' = ' . $db->quote('com_spmessage.message') );
			// Create a new query object.
			$query = $db->getQuery(true);
			...
			{
				// If succesfully remove Message add queued success message.
				$app->enqueueMessage(JText::_('The (com_spmessage.message) type alias was removed from the <b>#__ucm_content</b> table'));
			}
			...
			foreach ($message_ids as $message_id)
			{
				...
				$message_condition = array( $db->quoteName('ucm_type_id') . ' = ' . $message_id);
				// Create a new query object.
				$query = $db->getQuery(true);
		...
		// Remove spmessage assets from the assets table
		$spmessage_condition = array( $db->quoteName('name') . ' LIKE ' . $db->quote('com_spmessage%') );

		// Create a new query object.
		$query = $db->getQuery(true);
		$query->delete($db->quoteName('#__assets'));
		$query->where($spmessage_condition);
		$db->setQuery($query);
		$message_done = $db->execute();
		if ($message_done)
		{
			// If succesfully remove spmessage add queued success message.
	...
	public function preflight($type, JAdapterInstance $parent)
	{
		...
		if (JFile::exists(JPATH_ADMINISTRATOR . '/components/com_spmessage/helpers/PHPExcel.php'))
		{
			// We need to remove this old PHPExcel folder
			$this->removeFolder(JPATH_ADMINISTRATOR . '/components/com_spmessage/helpers/PHPExcel');
			// We need to remove this old PHPExcel file
			JFile::delete(JPATH_ADMINISTRATOR . '/components/com_spmessage/helpers/PHPExcel.php');
		}
		return true;
	}
	...
	public function postflight($type, JAdapterInstance $parent)
	{
		...
		if ($type === 'install')
		{
			...
			$message_Inserted = $db->insertObject('#__content_types', $message);
			...
			// Field to update.
			$fields = array(
				$db->quoteName('rules') . ' = ' . $db->quote('{"site.listmessage.access":{"1":1}}'),
			);
			// Condition.
			$conditions = array(
				$db->quoteName('name') . ' = ' . $db->quote('com_spmessage')
			);
			...
			// Condition.
			$conditions = array(
				$db->quoteName('element') . ' = ' . $db->quote('com_spmessage')
			);
		...
		if ($type === 'update')
		{
			...
			$query->from($db->quoteName('#__content_types'));
			$query->where($db->quoteName('type_alias') . ' LIKE '. $db->quote($message->type_alias));
			$db->setQuery($query);
			$db->execute();
			...
			if ($db->getNumRows())
			{
				...
				$message_Updated = $db->updateObject('#__content_types', $message, 'type_id');
			}
			else
			{
				$message_Inserted = $db->insertObject('#__content_types', $message);
