...
/**
 * Limitations Table class
 */
class SplimitationTableLimitation extends JTable
{
	/**
	...
	{
		parent::__construct('#__splimitation_limitation', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Limitation table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New limitation. A limitation created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('limitation', 'SplimitationTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SPLIMITATION_LIMITATION_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_splimitation.limitation.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_splimitation.limitation.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_splimitation');
