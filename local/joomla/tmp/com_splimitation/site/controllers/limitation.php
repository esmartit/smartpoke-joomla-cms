 ...
 */
class SplimitationControllerLimitation extends JControllerForm
{
	/**
	...
	{
		$this->view_list = 'listlimitation'; // safeguard for setting the return view listing to the default site view.
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_splimitation.limitation.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_splimitation.limitation.' . $recordId))
				{
					...
					if ($ownerId == $user->id)
					{
						if ($user->authorise('core.edit.own', 'com_splimitation'))
						{
							return true;
						}
					}
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Limitation', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_splimitation&view=limitations' . $this->getRedirectToListAppend(), false));
