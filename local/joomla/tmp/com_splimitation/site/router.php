...
/**
 * Routing class from com_splimitation
 *
 * @since  3.3
 */
class SplimitationRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_splimitation component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'listlimitation'))
		{
			if ($mId != (int) $query['id'] || $mView != $view)
			{
				if (($view === 'listlimitation'))
				{
					$segments[] = $view;
					$id = explode(':', $query['id']);
					if (count($id) == 2)
	...
	{		
		...
		switch($segments[0])
		{
			case 'listlimitation':
				...
				{
					$id = $this->getVar('limitation', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'splimitation')
	{
...
function SplimitationBuildRoute(&$query)
{
	$router = new SplimitationRouter;
	
	return $router->build($query);
}

function SplimitationParseRoute($segments)
{
	$router = new SplimitationRouter;
