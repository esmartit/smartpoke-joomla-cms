...
/**
 * Routing class from com_spnas
 *
 * @since  3.3
 */
class SpnasRouter extends JComponentRouterBase
{	
	/**
	 * Build the route for the com_spnas component
	 *
	...
	public function build(&$query)
	{
		...
		if (isset($view) && isset($query['id']) && ($view === 'listnas'))
		{
	...
	{		
		...
		switch($segments[0])
		{
			case 'listnas':
				...
				{
					$id = $this->getVar('nas', $segments[$count-1], 'alias', 'id');
					if($id)
					{
						$vars['id'] = $id;
					}
	...
	protected function getVar($table, $where = null, $whereString = null, $what = null, $category = false, $operator = '=', $main = 'spnas')
	{
...
function SpnasBuildRoute(&$query)
{
	$router = new SpnasRouter;
	
	return $router->build($query);
}

function SpnasParseRoute($segments)
{
	$router = new SpnasRouter;
