 ...
 * Spnas Route Helper
 **/
abstract class SpnasHelperRoute
{
	protected static $lookup;

	/**
	 * @param int The route of the Listnas
	 */
	public static function getListnasRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'listnas'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_spnas&view=listnas&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_spnas&view=listnas';
		}
		if ($catid > 1)
		{
	...
	/**
	 * Get the URL route for spnas category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_spnas&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_spnas');
		...
		if ($type)
		{
			...
			$params = JComponentHelper::getParams('com_spnas');
