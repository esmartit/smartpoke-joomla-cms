 ...
 * Nases Table class
 */
class SpnasTableNas extends JTable
{
	/**
	...
	{
		parent::__construct('#__spnas_nas', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Nas table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New nas. A nas created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('nas', 'SpnasTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SPNAS_NAS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_spnas.nas.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_spnas.nas.'.(int) $this->$k;
	}
